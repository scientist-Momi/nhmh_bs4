...
if (!empty($_POST['user_id'])) {
    ...
    $sql1 = mysqli_query($conn, "select * from nhmh_staff_db where unique_id = '$user_id' limit 1");
    ...
    if (mysqli_num_rows($sql1) > 0) {
        ...
        $sql = "update nhmh_staff_db set firstname = ?, lastname = ?, email = ?, phone = ?, photo = ?, address = ? where unique_id = ? ";
        $stmt = $conn->stmt_init();
        if (!$stmt->prepare($sql)) {
            echo json_encode([
                "status" => 401,
